...
K_THREAD_STACK_DEFINE(esp8266_rx_stack,
		      CONFIG_WIFI_ESP8266_RX_THREAD_STACK_SIZE);

/* RX thread work queue */
K_THREAD_STACK_DEFINE(esp8266_workq_stack,
		      CONFIG_WIFI_ESP8266_WORKQUEUE_STACK_SIZE);

struct k_thread esp8266_rx_thread;

K_SEM_DEFINE(sock_sem, 1, 1);
K_MUTEX_DEFINE(dev_mutex);
K_MUTEX_DEFINE(send_cmd_mutex);
...
#ifdef	CONFIG_WIFI_ESP8266_DEBUG_HEXDUMP
...
#else
#define	_hexdump(...)			do { } while (0)
#endif	/*CONFIG_WIFI_ESP8266_DEBUG_HEXDUMP*/
...
{
	u8_t msg[64];
	net_buf_linearize(msg, sizeof(msg), *buf, 0, len);
    msg[len] = '\0';
    //printk("ESP8266>> [%s]\n", msg);
}
...
static void esp8266_ip_addr_work(struct k_work *work)
{
    ...
    /*
     * tifler-20181101
     * XXX This work brings the esp8266 to abnormal state.
     *
     *     The root cause of the problem is calling send_at_cmd() at
     *     from worker thread while other thread is waiting for returning from
     *     k_sem_take(&foo_data.response_sem).
     *     send_at_cmd() resets foo_data.response_sem at the start of the func.
     *     It's weird implementation.
     *     At the moment, we use this driver only for testing purpose, I won't
     *     fix this problem.
     */
    return;
...
static void on_cmd_wifi_scan_resp(struct net_buf **buf, u16_t len)
{
	...
	/* mac */
	slen = delimiters[4] - delimiters[3];
	net_buf_linearize(temp, 32, *buf, delimiters[3], slen);
	temp[slen] = '\0';
	{
		char *p = &temp[1]; /* skip double-quote */
		for (i = 0; i < 6; i++) {
			p[2] = 0;
			result.mac[i] = strtol(p, NULL, 16);
			p += 3;
		}
	}
...
static void esp8266_rx(void)
{
	...
	static const struct cmd_handler handlers[] = {
		CMD_HANDLER("+CIUPDATE", atcmdecho_nosock),
		CMD_HANDLER("AT version", atcmdecho_nosock),
		CMD_HANDLER("SDK version", atcmdecho_nosock),
...
#ifdef  CONFIG_SOC_GCT_GDM7243I
#include <arch/delay.h>
#ifndef DMEM
#define DMEM(x)     (*(volatile unsigned *)(x))
#endif  /*DMEM*/
static void init_board(void)
{
#define GDM_GPIO_BASE           (0x10060000)
#define GDM_GPIO_GPG1_DIR       (GDM_GPIO_BASE+0x024)
#define GDM_GPIO_GPG1_DATA      (GDM_GPIO_BASE+0x064)
#define GDM_GPIO_GPG1_CFG       (GDM_GPIO_BASE+0x324)

    /* AUDIO_EN=1 */
#if 1
    DMEM(GDM_GPIO_GPG1_CFG) |= (1 << 5);
    DMEM(GDM_GPIO_GPG1_DATA) &= ~(1 << 5);
    DMEM(GDM_GPIO_GPG1_DIR) |= (1 << 5);
#endif  /*0*/

    LOG_DBG("GDM_GPIO_GPG1_DIR=0x%x\n", DMEM(GDM_GPIO_GPG1_DIR));
    LOG_DBG("GDM_GPIO_GPG1_DATA=0x%x\n", DMEM(GDM_GPIO_GPG1_DATA));
    LOG_DBG("GDM_GPIO_GPG1_CFG=0x%x\n", DMEM(GDM_GPIO_GPG1_CFG));
...
#define GDM_UART1_BASE          0x13100000
#define GDM_UART2_BASE          0x13110000
#define GDM_UART3_BASE          0x13120000

#define LCR(base)              ((base) + 0x08) /* line control register */
#define BAUD(base)             ((base) + 0x0c) /* baud rate register */
#define CR(base)               ((base) + 0x10) /* control register */

    LOG_DBG("GDM_UART1_LCR=0x%x\n", DMEM(LCR(GDM_UART1_BASE)));
    LOG_DBG("GDM_UART1_BAUD=0x%x\n", DMEM(BAUD(GDM_UART1_BASE)));
    LOG_DBG("GDM_UART1_CR=0x%x\n", DMEM(CR(GDM_UART1_BASE)));

    LOG_DBG("GDM_UART2_LCR=0x%x\n", DMEM(LCR(GDM_UART2_BASE)));
    LOG_DBG("GDM_UART2_BAUD=0x%x\n", DMEM(BAUD(GDM_UART2_BASE)));
    LOG_DBG("GDM_UART2_CR=0x%x\n", DMEM(CR(GDM_UART2_BASE)));

    LOG_DBG("GDM_UART3_LCR=0x%x\n", DMEM(LCR(GDM_UART3_BASE)));
    LOG_DBG("GDM_UART3_BAUD=0x%x\n", DMEM(BAUD(GDM_UART3_BASE)));
    LOG_DBG("GDM_UART3_CR=0x%x\n", DMEM(CR(GDM_UART3_BASE)));

}
#else
#define init_board(...)          do { } while (0)
#endif  /*CONFIG_SOC_GCT_GDM7243I*/
...
static void esp8266_reset_work(void)
{
	...
	if (k_sem_take(&foo_data.response_sem, MDM_CMD_TIMEOUT)) {
		printk("timed out waiting for device to become ready\n");
		return;
	}
#endif  /*CONFIG_SOC_GCT_GDM7243I*/
	...
	ret = send_at_cmd(NULL, "AT+GMR", MDM_CMD_TIMEOUT);
	if (ret < 0) {
		LOG_ERR("failed to set multiple socket support\n");
		return;
	}
...
NET_DEVICE_OFFLOAD_INIT(esp8266, CONFIG_WIFI_ESP8266_NAME,
